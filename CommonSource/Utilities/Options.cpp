	...
	for(groupIter = mOptionGroups.begin(); groupIter != mOptionGroups.end(); ++groupIter) {
		...
		for(optionIter = groupIter->Options.begin(); optionIter != groupIter->Options.end(); ++optionIter) {
			...
			if(optionIter->HasDefaultValue) {
				...
				} else if(optionIter->DefaultValue.is_type<unsigned short>()) {
						sb << (unsigned short)optionIter->DefaultValue;
			...
			if(ovMapIter->second.StoreValue) {
				...
				if(i < LAST_INDEX) {
					...
					if(checkMapIter == mOptionsMap.end()) {
						...
						} else if(ovMapIter->second.VariantValue.is_type<unsigned short>()) {

							const unsigned short uint16 = (unsigned short)strtoul(val.c_str(), &end_ptr, 10);
							unsigned short* varValue = (unsigned short*)ovMapIter->second.pValue;
							*varValue = uint16;
						...
						} else if(ovMapIter->second.VariantValue.is_type<vector<unsigned short> >()) {

							vector<unsigned short>* pVectorValue = (vector<unsigned short>*)ovMapIter->second.pValue;
							const unsigned short uint16 = (unsigned short)strtoul(val.c_str(), &end_ptr, 10);
							pVectorValue->push_back(uint16);
