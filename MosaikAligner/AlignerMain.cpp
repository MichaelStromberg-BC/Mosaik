...
int main(int argc, char* argv[]) {
	...
	COptions::AddOption("-rbm",                        "stores both mates in the unaligned file",    settings.RecordBothMates,                                       pReportingOpts);
	...
	// check that the rbm option is used in coordination with the rur option
	if(settings.RecordBothMates && !settings.RecordUnalignedReads) {
		errorBuilder << ERROR_SPACER << "The record unaligned reads option (-rur) needs to be used in conjunction with the record both mates (-rbm) option." << endl;
		foundError = true;
	}
	...
	// enable unaligned read reporting with both mates
	if(settings.RecordBothMates) ma.EnableUnalignedReadReportingBothMates();
	...
	if(settings.RecordUnalignedReads) {
		cout << "- Reporting all unaligned reads to " << settings.UnalignedReadsFilename << ".";
		if(settings.RecordBothMates) cout << " Both mates will be reported even if aligned.";
		cout << endl;
	}
