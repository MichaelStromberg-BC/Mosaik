...
, mSW(CPairwiseUtilities::MatchScore, CPairwiseUtilities::MismatchScore, CPairwiseUtilities::GapOpenPenalty, CPairwiseUtilities::GapExtendPenalty)
  //, mLocalSW(CPairwiseUtilities::MatchScore, CPairwiseUtilities::MismatchScore, CPairwiseUtilities::GapOpenPenalty, CPairwiseUtilities::LocalGapExtendPenalty)
...
void CAlignmentThread::AlignReadArchive(MosaikReadFormat::CReadReader* pIn, MosaikReadFormat::CAlignmentWriter* pOut, FILE* pUnalignedStream, uint64_t* pReadCounter, bool isPairedEnd) {
	...
	while(true) {
		...
		if(numMate1Bases != 0) {
			...
			// align the read and calculate the alignment qualities
		...
		// =========================
		// store the unaligned reads
		// =========================

		if(mFlags.IsReportingUnalignedReads) {

			bool reportBothReads = (mFlags.ReportingBothMatesWhenUnaligned && areBothMatesPresent && (!isMate1Aligned || !isMate1Aligned));

			// write mate 1
			if(reportBothReads || (!isMate1Aligned && (numMate1Bases != 0))) {
				pthread_mutex_lock(&mReportUnalignedMate1Mutex);
				CMosaikString qualities = mr.Mate1.Qualities;
				qualities.Increment(33);
				...
				fprintf(pUnalignedStream, "@%s (mate 1, length=%u%s)\n%s\n+\n%s\n", mr.Name.CData(), 
					(isUsingSOLiD ? numMate1Bases + 1 : numMate1Bases), (isMate1Aligned ? ", aligned" : ""), 
					mr.Mate1.Bases.CData(), qualities.CData());

				pthread_mutex_unlock(&mReportUnalignedMate1Mutex);
			}

			// write mate 2
			if(reportBothReads || (!isMate2Aligned && (numMate1Bases != 0))) {
				pthread_mutex_lock(&mReportUnalignedMate2Mutex);
				CMosaikString qualities = mr.Mate2.Qualities;
				qualities.Increment(33);
				...
				fprintf(pUnalignedStream, "@%s (mate 2, length=%u%s)\n%s\n+\n%s\n", mr.Name.CData(), 
					(isUsingSOLiD ? numMate2Bases + 1 : numMate2Bases), (isMate2Aligned ? ", aligned" : ""), 
					mr.Mate2.Bases.CData(), qualities.CData());
...
bool CAlignmentThread::RescueMate(const LocalAlignmentModel& lam, const CMosaikString& bases, const unsigned int uniqueBegin, const unsigned int uniqueEnd, const unsigned int refIndex, Alignment& al) {
	...
	// try to help alignments with single contiguous deletions pass the mismatch criteria
	//const double percentMismatch = (double)al.NumMismatches / (double)al.QueryLength;
	/*
	if((mFilters.UseMismatchFilter && (al.NumMismatches > mFilters.MaxNumMismatches)) || (mFilters.UseMismatchPercentFilter && (percentMismatch > mFilters.MaxMismatchPercent))) {

		const unsigned int alignmentLength = al.Reference.Length();
		const char* pAlQuery = al.Query.CData();

		unsigned short numQueryGaps     = 0;
		unsigned short numQueryGapBases = 0;

		for(unsigned int i = 0; i < alignmentLength; i++) {
			if(pAlQuery[i] == '-') {
				numQueryGaps++;
				unsigned int pos = i;
				while((pos < alignmentLength) && (pAlQuery[pos] == '-')) {
					pos++;
					numQueryGapBases++;
				}
				i = pos - 1;
		...
		if(numQueryGaps == 1) al.NumMismatches -= numQueryGapBases;
	}
	*/
